...
class FaceRecognizerViewModel @Inject constructor(
    ...
    companion object {
        private const val TAG = "FaceFlow"
    }
    ...
    fun onImagesCaptured(bitmaps: List<Bitmap>, mode: FaceFlowMode) {
        ...
        viewModelScope.launch {
            try {
                ...
                val embeddingTemplates = computeEmbeddingUseCase(bitmaps)

                val timestamp = java.text.SimpleDateFormat("yyyyMMdd_HHmm", java.util.Locale.getDefault())
                    .format(java.util.Date())

                val newUser = User(fullName = "User_$timestamp", embeddingTemplates = embeddingTemplates)
                ...
                Log.e(TAG, "Failed to process capture", e)
    ...
    fun verifyUser() {
        ...
        val embeddingTemplates = currentUser.embeddingTemplates

        viewModelScope.launch {
            try {
                _uiState.value = FaceUiState.Loading
                val user = verifyUserUseCase(embeddingTemplates)
